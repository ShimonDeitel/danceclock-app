    ...
    private func row(_ entry: SessionEntry) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Duration (min): \(entry.value1)  \u{00B7}  Energy (1-5): \(entry.value2)")
                    .font(.caption)
...
struct EntrySheet: View {
    ...
    @State private var option: String = DanceclockOptions.all.first ?? ""
            ...
            Form {
                Section("Style") {
                    Picker("Style", selection: $option) {
                        ForEach(DanceclockOptions.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
